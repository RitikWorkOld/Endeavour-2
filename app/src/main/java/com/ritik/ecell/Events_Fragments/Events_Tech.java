...
public class Events_Tech extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        final View view = inflater.inflate(R.layout.fragment_events__tech,container,false);

        no_new_notifications = view.findViewById(R.id.no_new_events);

        recyclerView = view.findViewById(R.id.rv_events_tech);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(container.getContext()));
        recyclerView.showIfEmpty(no_new_notifications);

        databaseReference = FirebaseDatabase.getInstance().getReference().child("eventstech");
